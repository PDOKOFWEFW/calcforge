...
export default function CityCard({ city }: { city: City }) {
  ...
  const total = pi + (city.medianHomePrice * city.propertyTaxRate)/100/12 + city.avgHOA;

  return (
    <Link href={`/mortgage/${city.slug}`}
      ...
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-stone-400 mt-1">{city.state} · {city.population.toLocaleString()} residents</p>
        </div>
      ...
      <div className="space-y-2 mb-4">
        {[
          ...
          <div key={s.label} className="flex items-center justify-between text-sm">
            ...
            <span className="font-medium text-stone-700 tabular-nums">{s.value}</span>
          </div>
        ))}
      </div>
      <div className="bg-navy-gradient rounded-xl px-4 py-3 flex items-center justify-between">
        <span className="text-white/60 text-xs">Est. monthly</span>
        <span className="text-white font-display text-xl tabular-nums">{formatCurrency(total)}</span>
      </div>
      <p className="text-[10px] text-stone-400 text-right mt-1.5">20% down · 30yr · {city.avgRate30yr}% APR</p>
    </Link>
  );
}
