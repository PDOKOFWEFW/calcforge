...
export default function MortgageCalculator({ city }: Props) {
  ...
  const onNumericInput = useCallback(
  ...
  return (
    <div className="bg-white rounded-3xl border border-stone-200 shadow-lg overflow-hidden">
      ...
      <div className="bg-navy-gradient px-6 py-4 flex items-center justify-between">
        <div>
          ...
          <h2 className="text-white font-semibold text-base">
            {city.name}, {city.stateAbbr} — Pre-filled with local data
          </h2>
        </div>
        <div className="hidden sm:flex items-center gap-1.5 bg-white/10 rounded-xl px-3 py-1.5">
          <span className="w-2 h-2 rounded-full bg-teal-400 animate-pulse" />
          <span className="text-white/70 text-xs">Live calculation</span>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-[1fr_420px]">
        ...
        {/* ── LEFT: Inputs ── */}
        <div className="p-6 space-y-6 border-b lg:border-b-0 lg:border-r border-stone-100">
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-1 bg-stone-50 border border-stone-200 rounded-lg px-2.5 py-1">
                ...
                <input
                  ...
                  value={homePrice}
                  onChange={onNumericInput(setHomePrice, 50000, 10000000)}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-1 bg-stone-50 border border-stone-200 rounded-lg px-2.5 py-1">
                <input
                  ...
                  min={0} max={80} step={1}
                  value={downPct}
                  onChange={onNumericInput(setDownPct, 0, 80)}
            ...
            <div className="flex justify-between text-xs text-stone-400 mt-1">
              ...
              <span className={`text-xs px-1.5 py-0.5 rounded-md font-medium ${
                ltvPct > 80 ? "bg-gold-100 text-gold-700" : "bg-teal-100 text-teal-700"
              }`}>
        ...
        <div className="p-6 bg-stone-50/50 flex flex-col gap-4">
          ...
          <div className="bg-white rounded-2xl border border-stone-200 shadow-sm p-5 text-center">
            ...
            <p className="font-display text-5xl text-navy tabular-nums leading-none mb-1" key={result.totalMonthlyPayment}>
            ...
            <div className="flex rounded-full overflow-hidden h-2 mt-4 gap-0.5">
              <div className="bg-brand-600 transition-all duration-500" style={{ width: `${piPct}%` }} title={`P&I ${piPct}%`} />
              <div className="bg-gold-500 transition-all duration-500" style={{ width: `${taxPct}%` }} title={`Tax ${taxPct}%`} />
              {hoaPct > 0 && <div className="bg-teal-500 transition-all duration-500" style={{ width: `${hoaPct}%` }} title={`HOA ${hoaPct}%`} />}
            </div>
            <div className="flex items-center justify-center gap-4 mt-2.5">
              <span className="flex items-center gap-1.5 text-[11px] text-stone-500">
                <span className="w-2 h-2 rounded-full bg-brand-600" />P&I {piPct}%
              </span>
              <span className="flex items-center gap-1.5 text-[11px] text-stone-500">
                <span className="w-2 h-2 rounded-full bg-gold-500" />Tax {taxPct}%
              </span>
              {hoaPct > 0 && (
                <span className="flex items-center gap-1.5 text-[11px] text-stone-500">
                  <span className="w-2 h-2 rounded-full bg-teal-500" />HOA {hoaPct}%
                </span>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-stone-200 shadow-sm divide-y divide-stone-100">
            {[
              ...
              ...(includeHOA && city.avgHOA > 0
                ? [{ label: "HOA Fee", value: result.monthlyHOA, color: "text-teal-600" }]
                : []),
            ].map(row => (
              <div key={row.label} className="flex items-center justify-between px-4 py-3">
                <span className="text-sm text-stone-600">{row.label}</span>
                <span className={`text-sm font-semibold font-mono tabular-nums ${row.color}`}>
          ...
          <div className="bg-gold-50 border border-gold-200 rounded-xl p-3.5 flex gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-gold-700 leading-relaxed">
                To comfortably afford {formatCurrency(result.totalMonthlyPayment)}/mo, you&apos;d need roughly{" "}
                <strong>{formatCurrency((result.totalMonthlyPayment / 0.28) * 12)}/year</strong> gross income
                (28% rule).
              </p>
            </div>
          </div>
    ...
    <div className="border-t border-stone-100">
      ...
      >
        <span className="flex items-center gap-2">
          ...
          Full Amortization Schedule ({termYears} years)
        </span>
